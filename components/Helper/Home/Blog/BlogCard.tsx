...
const BlogCard = ({blog}:Props) => {
  ...
  return (
    <div className='bg-indigo-900 rounded-md overflow-hidden'>
      <img src={image} alt={title} width={300} height={300} className='w-full h-[250px] object-cover'/>
      <div className='p-6'>
        <p className='rounded-sm px-6 py-1.5 bg-rose-500 text-white w-fit'>News</p>
        <h1 className='sm:text-lg text-base cursor-pointer hover:underline transition-all duration-200 mt-6 mb-2 text-gray-300 leading-[1.6rem] font-bold'>{title}</h1>
